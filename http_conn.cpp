...
/* 当得到一个完整、正确的HTTP请求时，我们就分析目标文件的属性。 如果目标文件存在、对所有用户可读，且不是目标，
 * 则使用mmap将其映射到内存地址m_file_address处，并告诉调用者读取文件成功*/
...
bool http_conn::write() {
    ...
    while (1) {
        ...
        if(temp <= -1) {
            /* 如果TCP写缓冲没有空间，则等待下一轮EPOLLOUT事件。虽然在此期间，服务器无法立刻接收到
             * 同一客户的下一个请求，但者可以保证连接的完整性*/
            if(errno == EAGAIN) {
